...
varnames <- vars[sapply(vars, function(x) grepl("*Diff.*KPP", x))]

varnames <- c("IsWinnerA", varnames)
train_set <- tt_data$train[, names(tt_data$train) %in% varnames]
test_set <- tt_data$test[, names(tt_data$test) %in% varnames]
glm_fit <- glm(IsWinnerA ~ (.)^2, family = "binomial", data = train_set)
predictions <- predict(glm_fit, newdata = test_set)
is_winner_predicted <- as.factor(ifelse(predictions[[i]] > 0, "TRUE", "FALSE"))
